...
public:
    explicit MemRing(size_t size)
        ...
        , _size(0)
        , _size_mask(0) {
        if(!size) throw AVException("memRing: wat?");
        ...
        _buff = reinterpret_cast<T*>(av_malloc(_size * sizeof(T)));

        if(!_buff) throw AVException("memRing: cannot allocate memory");
    }
    ...
    MemRing(const MemRing&) = delete;
    MemRing& operator=(const MemRing&) = delete;
    MemRing(MemRing&&) = delete;
    MemRing& operator=(MemRing&&) = delete;

    size_t readSpace() {
        size_t w = _w_ptr.load();
        size_t r = _r_ptr.load();
    ...
    size_t pull(T* data, size_t len) {
        ...
        if(!read_space) return 0;

        size_t to_read = len > read_space ? read_space : len;
        size_t n0, n1, n2, r_ptr = _r_ptr.load();
    ...
    size_t push(T* data, size_t len) {
        ...
        if(!write_space) return 0;

        size_t to_write;
        size_t n0, n1, n2, w_ptr = _w_ptr.load();
...
private:
    T* _buff;
    std::atomic<size_t> _w_ptr;
    std::atomic<size_t> _r_ptr;
