...
void AVObject::connectInput(AVObject* object, bool recursive) {
    ...
    _input = object;
    if(recursive) object->connectOutput(this, false);
}
...
void AVObject::connectOutput(AVObject* object, bool recursive) {
    ...
    if(_output) throw AVException("connectOutput: already connected");

    _output = object;
    if(recursive) object->connectInput(this, false);
}

void AVObject::disconnectInput(AVObject* object, bool recursive) {
    if(_input != object) throw AVException("disconnectInput: programming error");

    _input = 0;
    if(recursive) object->disconnectOutput(this, false);
}

void AVObject::disconnectOutput(AVObject* object, bool recursive) {
    if(_output != object) throw AVException("disconnectOutput: programming error");
