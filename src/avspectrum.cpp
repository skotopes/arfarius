...
AVSpectrum::AVSpectrum(size_t window_size, WindowType window_type, float threshold)
    ...
    if(!_in || !_out) {
        ...
        _out = nullptr;
        fftw_mutex.unlock();
        throw AVException("AVSpectrum: cannot allocate FFTW memory");
    }
    _plan = fftwf_plan_dft_1d(
        _window_size, _in, _out, FFTW_FORWARD, FFTW_MEASURE | FFTW_DESTROY_INPUT);
    fftw_mutex.unlock();
}
...
void AVSpectrum::_windowHann() {
    for(size_t x = 0; x < _window_size; x++) {
        _in[x][0] = _in[x][0] * (0.5 - 0.5 * cosf(2 * M_PI * x / (_window_size - 1)));
    }
}

void AVSpectrum::_windowHamming() {
    for(size_t x = 0; x < _window_size; x++) {
        _in[x][0] = _in[x][0] * (0.54 - 0.46 * cosf(2 * M_PI * x / (_window_size - 1)));
    }
}

void AVSpectrum::_windowBlackman() {
    for(size_t x = 0; x < _window_size; x++) {
        _in[x][0] = _in[x][0] * (0.42659 - 0.49656 * cosf(2 * M_PI * x / (_window_size - 1)) +
...
void AVSpectrum::_windowBlackmanHarris() {
    for(size_t x = 0; x < _window_size; x++) {
        _in[x][0] = _in[x][0] * (0.35875 - 0.48829 * cosf(2 * M_PI * x / (_window_size - 1)) +
...
void AVSpectrum::_postProcess() {
    ...
    for(size_t x = 0; x < _window_size / 2; x++) {
        float magnitude =
            sqrtf(_out[x][0] * _out[x][0] + _out[x][1] * _out[x][1]) * sqrtf(2) / _window_size;
...
void AVSpectrum::_processDb() {
    ...
    _low_rms = _low_cnt > 0 ? 20.0f * log10f(_low_rms / _low_cnt) : kMinDb;
    _mid_rms = _mid_cnt > 0 ? 20.0f * log10f(_mid_rms / _mid_cnt) : kMinDb;
    _high_rms = _high_cnt > 0 ? 20.0f * log10f(_high_rms / _high_cnt) : kMinDb;

    if(_low_rms > _threshold) {
        _low_rms = 1.0f - (_threshold / _low_rms);
    } else {
        _low_rms = 0;
    }

    if(_mid_rms > _threshold) {
        _mid_rms = 1.0f - (_threshold / _mid_rms);
    } else {
        _mid_rms = 0;
    }

    if(_high_rms > _threshold) {
        _high_rms = 1.0f - (_threshold / _high_rms);
    } else {
        _high_rms = 0;
    }
}
...
void AVSpectrum::_processLinear() {
    _low_rms = _low_cnt > 0 ? _low_rms / _low_cnt : 0;
    _mid_rms = _mid_cnt > 0 ? _mid_rms / _mid_cnt : 0;
    _high_rms = _high_cnt > 0 ? _high_rms / _high_cnt : 0;
}
